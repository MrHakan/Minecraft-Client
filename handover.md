...
- **Current main run:** [35853612285](https://github.com/MrHakan/Minecraft-Client/actions/runs/35853612285) on `04e30e8` passed build, unit tests and client game tests. The production-addon task has remained active for over 15 minutes without a result, so installation, restart persistence and removal are still **not accepted**. The workflow now bounds this step to 15 minutes and preserves its logs with the following `always()` artifact upload.
...
1. Let the 15-minute-bounded production step finish and inspect its launch logs and stage markers.
