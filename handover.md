...
- **Latest completed full run:** [35851703294](https://github.com/MrHakan/Minecraft-Client/actions/runs/35851703294) on `6e9ce144830c7fee1dce112211fc5282f169c85f` built with Java 25, reported **826 unit tests with 0 failures/errors/skips**, and passed client game tests. Its production-addon step failed before launch because `remapJar` does not exist under the project's 26.2 non-obfuscated Loom plugin. Follow-up commit `04e30e8` switches to the ordinary `jar` output and fixture JARs, which need no remapping for this Minecraft version.
- **Current main follow-ups:** [35853612285](https://github.com/MrHakan/Minecraft-Client/actions/runs/35853612285) on `04e30e8` passed build, unit tests and client game tests. Its first production launch loaded the main mod and addon but stopped during resource reload after texture atlases; no title screen or stage marker appeared. It was cancelled after 17 minutes and its `latest.log` was retained. [35856112167](https://github.com/MrHakan/Minecraft-Client/actions/runs/35856112167) on `5c12203` also passed build and game tests, then entered the same production step; it is being superseded by a fixture watchdog change based on the captured log. Neither run accepts addon installation yet.
...
- **Production-like addon test:** intended to launch the production client and addon fixtures three times in one isolated `mods/` directory, checking initial settings save, addon and base-setting persistence after restart, then safe addon removal. Run 35851703294 did not launch it because its task configuration referenced `remapJar`; run 35853612285 showed the production client and addon loading but hung before the title screen. The monitor fixture now has a 120-second per-stage watchdog that writes JVM thread dumps and exits nonzero if that happens again; the workflow has a separate 15-minute step limit. The next main run must produce all three `ok` markers before this is accepted. Missing-dependency loader messaging remains manual.
...
1. Run the per-stage watchdog and 15-minute-bounded production step; inspect launch logs, thread dumps and stage markers.
2. Fix any launcher or settings-persistence defect exposed by the bounded run; repeat the full main pipeline.
