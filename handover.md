...
- **Latest green code-bearing run:** [35861603084](https://github.com/MrHakan/Minecraft-Client/actions/runs/35861603084) on `ff226f84650b47bf910a268feabdd22e64d2d4bd`. Java 25 build, **826 unit tests with 0 failures/errors/skips**, all client game tests, runtime mixin verification, inventory regression controls, rolled-log scan and artifacts passed. The production addon install/restart/removal test also passed all three marker checks. This documentation-only reconciliation receives a separate full main-branch CI run.
...
- **AutoGrind:** planning remains separate from `GrindExecutor`. `.grind <item> [count]` reports the plan; `.grind run log N` executes bounded nearby log gathering. It uses the shared ScannerService budget, RotationService, vanilla game-mode block breaking and inventory-confirmed drop completion. Out-of-reach targets and uncollected drops pause as `NEEDS_MOVEMENT` with coordinates and an honest movement-unavailable message. Stopping and starting again replans against live inventory.
- **AutoGrind evidence:** the full 26.2 client game-test suite passed in run 35861603084. `ModuleBehaviourGameTest.grindExecutesNearbyLogs` covers aiming from a wrong initial view, real block breaking and drop pickup, manual movement/resume, stopping and restarting after a partial amount, and inventory-confirmed completion. A distant-target test verifies the movement-unavailable pause and manual-resume path. Crafting, smelting, other resource executors and AutoGrind pathfinding remain unsupported; no compatible installed Baritone JAR is claimed.
- **NoFall and Jesus:** NoFall waits for predicted collision with a real support surface and sends at most one grounded-status packet per fall; its description states that servers may still apply damage. Jesus derives its water target from the live fluid surface and keeps the separate lava lift. `RemainingSafetyGameTest` measures integrated-server fall damage against a normal-fall control and checks shallow/deep water surface behavior. These tests passed in run 35861603084; this does not establish broad multiplayer or anti-cheat acceptance.
- **Production-like addon test:** run 35861603084 launched the built client `jar` with a separately packaged addon fixture in an isolated mods directory, saved addon and NoFall settings, restarted with the addon still installed, then removed it and verified the base config still loaded. All three marker checks passed. The fixture observes Fabric's client-started event before the first tick, avoiding the slow title-screen blur render on software OpenGL. Missing-dependency loader messaging remains manual.
- **Other release evidence:** CI runs four inventory regression controls, Java 25 build and unit tests, generated documentation, runtime mixin verification, full client game tests, production addon install/restart/removal, and a rolled-log failure scan. Dedicated-server tests remain opt-in behind the EULA flag and are not coverage unless explicitly run.
...
- **Local toolchain:** this workspace has Java 17 and cannot reach the Gradle distribution download, so Java 25 validation relies on GitHub Actions.

### Current release-validation focus

1. Confirm the full CI run for this documentation-only reconciliation is green; then resume the existing six-hour maintenance automation without changing its schedule.
2. If a compatible Baritone 26.2 JAR is installed, test `.goto`, `.goto stop`, `.grind run` completion/cancellation and zero public-chat leakage.
3. Exercise natural fishing and inventory automation with latency/full-inventory contention.
4. Manually inspect TargetHUD face/hat compositing, ESP/nametag geometry, waypoint beams, trajectories, rainbow phase, HUD scale and light/AMOLED/high-contrast themes. Keep dedicated-server tests opt-in; do not accept the EULA on the owner's behalf.
...
| 5 | Rotations | Implemented: None/Client/Smooth with per-tick yaw/pitch step limits and one-winner-per-tick arbitration, now driven by two consumers - Aura and the `.look` command, via `LookController`, which holds a one-shot caller's aim open across ticks and lets go on arrival or a budget. No further module consumer is planned: every remaining candidate would be aim assistance, and TriggerBot by definition fires at what is already under the crosshair rather than turning to it. The **silent** half stays deliberately unimplemented: hiding rotation from the player's own view while sending it to the server exists to defeat server-side checks |
...
| 8 | Animations | Implemented: global controls, notification motion, the target HUD's eased bar, module list row slide/fade, and the ClickGUI's category stripe plus a content veil that covers opening, changing category and paging - the screen is rebuilt for all three. Only decoration animates: every row and category is a vanilla widget that hit-tests against its own bounds, so nothing clickable is ever drawn away from where it answers clicks. `ScreenTransition` is pure and unit tested, and the theme's reduced-motion switch is the same path with a duration of zero rather than a branch |
...
| 12 | Themes | Implemented: eight presets, guarded import/export, high contrast/reduced motion, a WCAG contrast guard and a HUD scale. A separate *screen* scale is **deliberately not added**: Minecraft's own GUI scale already scales the mod's screens, so a second control beside it would be a switch that forwards to a vanilla switch - the AutoJump case. The HUD is the mod's own drawing and is the half vanilla does not cover |
