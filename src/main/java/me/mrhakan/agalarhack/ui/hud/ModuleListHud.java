...
public final class ModuleListHud {
    ...
    public void render(GuiGraphicsExtractor g, Minecraft mc) {
        ...
        int max = Math.min((int) options.getNumberSetting("maximumRows", 32),
                Math.max(0, (g.guiHeight() - 8 - headerHeight) / Math.max(1, rowHeight)));
        var entries = manager.getModuleList().stream()
                .filter(module -> module.isToggled() && module.getBooleanSetting("showInHud", true))
        ...
        String countText = rows.size() == entries.size() ? rows.size() + " enabled" : rows.size() + "/" + entries.size() + " enabled";
        int contentWidth = rows.stream().mapToInt(ModuleListModel.Row::width).max().orElse(0);
        if (showCount) contentWidth = Math.max(contentWidth, mc.font.width(countText));
        ...
        var motionByText = new HashMap<String, RowAnimations.Row>();
        for (var motion : placed) motionByText.put(motion.id(), motion);

        if (showCount) {
            int countX = right ? x + width - padding - mc.font.width(countText) : x + padding;
            g.text(mc.font, countText, countX, y, ClientUiTheme.MUTED, shadow);
        }
        ...
        for (var row : rows) {
            ...
            var motion = motionByText.get(row.text());
